...
def test_bin_mz_not_normalized_binning_and_clamp_and_padding(tmp_path):
    ...
    # mz=55 -> 0
    # mz=149 -> floor(99/10)=9
    # mz=200 -> clamp to 150 -> floor(100/10)=10 -> then clamp to 9
    npz = _make_npz(tmp_path, [[50.0, 55.0, 149.0, 200.0], [60.0]])
    out = bm.bin_mz(str(npz), bin_size=10.0, mz_min=50.0, mz_max=150.0, is_normalized=False)

    assert out.shape[0] == 2
    assert out.shape[1] == 4  # pad 到 max_len=4
    ...
    row1 = out[1].tolist()
    assert row1[0] == 1  # (60-50)/10=1
    assert row1[1] == 0  # padding -> valid_mask False -> 0
    assert row1[2] == 0
    assert row1[3] == 0
